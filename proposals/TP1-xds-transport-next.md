...
* Context parameters from the URL, in the above example
  ...
  i.e. they should avoid the `xds.node.` or `xds.resource.` prefixes.
...
*   When mapping resource identities to alternative transports to the xDS transport, a URI structure aids in this transposition. It’s easy to see how `xdstp://some-authority.com/listener/foo` can be served by a CDN `http://static.some-authority.com/listener/foo`.
...
To overcome this limitation, we have a native URI transform from `xdstp://&lt;some UDPA URI>` schema to `http[s]://`. This involves simply replacing the scheme with `http[s]` and ensuring that the authority is a DNS resolvable entity.
